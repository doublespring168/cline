...
const SapAiCoreModelPicker: React.FC<SapAiCoreModelPickerProps> = ({
	...
	useEffect(() => {
		...
		if (matchingDeployment) {
			...
			if (
				!selectedDeploymentId ||
				matchingDeployment.deploymentId !== selectedDeploymentId
			) {
	...
	const handleModelChange = (e: any) => {
		...
		const deployment = sapAiCoreModelDeployments.find(
			(d) => d.modelName === newModelId,
		);
	...
	const categorizedModels = useMemo(() => {
		...
		// Models that are both deployed AND supported in coderX
		const deployedModelNames = sapAiCoreModelDeployments.map(
			(d) => d.modelName,
		);
		const deployedAndSupported = deployedModelNames.filter(
			(deployedModel: string) => allSupportedModels.includes(deployedModel),
		);

		// Models that are supported in coderX but NOT deployed
		const supportedButNotDeployed = allSupportedModels.filter(
			(supportedModel: string) => !deployedModelNames.includes(supportedModel),
		);

		const deployed: CategorizedModel[] = deployedAndSupported.map(
			(modelName: string) => ({
				id: modelName,
				isDeployed: true,
		...
		const supported: CategorizedModel[] = supportedButNotDeployed.map(
			(id: string) => ({
				id,
				isDeployed: false,
