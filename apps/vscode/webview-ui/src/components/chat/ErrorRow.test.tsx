...
describe("ErrorRow", () => {
	...
	it("shows an authentication error as a normal API-key error without an account sign-in action", async () => {
		...
		expect(screen.queryByText("Sign in to coderX")).not.toBeInTheDocument();
	});

	it("renders diff and ignore errors", () => {
		const { rerender } = render(
			<ErrorRow errorType="diff_error" message={message} />,
		);
		expect(
			screen.getByText(/search patterns that don't match/),
		).toBeInTheDocument();

		rerender(
			<ErrorRow
				errorType="clineignore_error"
				message={{ ...message, text: "/blocked.txt" }}
			/>,
		);
