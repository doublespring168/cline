...
/**
 * Interactive Playwright launcher for the coderX VS Code extension.
 *
 * Overview:
 *  - Starts the mock coderX API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the coderX extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the coderX sidebar.
 ...
 *        npm run test:e2e:ui
 *
 *   3. VS Code will launch with the coderX extension loaded and gRPC recording enabled.
 *
...
async function main() {
	...
	console.log("VSCode with coderX extension is now running!");
	...
	async function teardown() {
		...
		try {
			...
			console.log(
				`We could teardown interactive playwright properly, error:${e}`,
			);
		}
