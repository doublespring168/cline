...
当前只有 VS Code 形态：任务数据根来自 `src/extension.ts` 调用 `HostProvider.initialize(..., context.globalStorageUri.fsPath)`；普通设置和密钥由共享文件存储写入 `${CLINE_DIR || ~/.cline}/data`。因此实际绝对路径仍可能受扩展 ID、VS Code Profile 和 `CLINE_DIR` 影响，不应在业务代码中硬编码。
...
`saveApiConversationHistory()` 现在只执行本地原子写入。原 `syncWorker` 队列、S3/R2/Azure adapters、`ClineBlobStorage`、Backfill、远程配置和 Prompt Upload 入口均已删除，因此 `api_conversation_history.json`、`ui_messages.json`、附件和工具记录都不会由项目代码自动上传到 Cline 云端或对象存储。

需要注意：模型请求本身仍会把当前 Context 发送给用户选择的模型 Provider，这是完成推理所必需的网络请求，不属于“聊天记录云同步”。MCP、Hooks 或用户自定义工具也可能按自身配置访问外部服务，应分别审计。
...
- usage：只更新 `api_req_started` 的 token/cost JSON，不进入 API message content，也不再上报遥测。
...
删除任务通常移除四个已知文件和空目录；若目录中还有 Focus Chain、临时文件或未来新增附件，目录仍可能残留。当前已无内置远端 Blob 和同步队列，合规删除重点是本地 task 目录、归档/导出目录，以及用户自行配置的 MCP、Hook 或外部模型服务。
