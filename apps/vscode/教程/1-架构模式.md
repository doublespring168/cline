...
> 分析日期：2026-07-15（已同步单一 VS Code 插件形态）

## 1. 结论先行

这个项目不是只采用了某一种教科书式架构。更准确的结论是：

> **它以“模块化单体”为代码组织基础，以“端口与适配器”隔离 VS Code 能力和业务核心，以 ProtoBus 连接 Webview 与 Extension Host；在 Agent 核心内部，又组合了事件驱动、状态机、策略、工厂、命令和注册表等模式。**

按重要性排序，可以概括为：

1. **主架构：分层的模块化单体（Layered Modular Monolith）**
2. **宿主边界：六边形架构 / 端口与适配器（Hexagonal / Ports and Adapters）**
3. **UI 通信：协议驱动的消息总线架构（ProtoBus）**
...
本次分析覆盖当前目录中的 TypeScript、TSX、JavaScript、构建脚本、Proto 定义和测试源码。独立 Core、外部宿主、独立终端和 gRPC 回放测试平台已经删除。
...
| `src/services` | 86 | MCP、浏览器、模型专属认证、本地日志、搜索等服务 |
| `src/hosts` | - | VS Code 宿主适配器 |
...
`src/generated` 与 `webview-ui/src/services/grpc-client.ts` 属于构建时生成物；它们由 `proto/**/*.proto` 和三个生成脚本决定。当前生成器只输出共享消息、Webview Client、VS Code Handler Map 和 VS Code HostBridge 服务表。
...
- `src/common.ts` 完成插件初始化：文件存储、本地日志、Webview、MCP、工作区、Hooks 和清理任务。
...
项目不再包含第二个 Core 组合根。VSIX 安装后只由 VS Code Extension Host 加载 `dist/extension.js`，不会监听 Core gRPC 端口或启动配套宿主进程。
...
| 层 | 关键目录/文件 | 职责 |
| --- | --- | --- |
| 组合根与生命周期 | `src/extension.ts`、`src/common.ts` | 装配 VS Code 实现、启动和释放资源 |
...
| 应用层 | `src/core/controller`、`Controller` | RPC 用例入口、任务生命周期、模型专属认证、模型配置和状态协调 |
...
| 适配器与基础设施层 | `src/hosts`、`src/integrations`、`src/services`、`src/core/api/providers` | VS Code、模型厂商、MCP、浏览器、Git、终端和本地日志 |
| 共享契约层 | `src/shared` | 前后端共同类型、消息、转换函数、纯逻辑和存储实现 |

### 4.1 为什么称为“模块化单体”

`src/core`、`services`、`integrations`、`hosts` 虽然模块很多，但最终由 esbuild 打成一个 `dist/extension.js`。模块之间是本地函数和对象调用，而不是独立部署的网络服务。

所以它具备“按领域拆模块”的单体特征，而不是微服务。ProtoBus 只跨越 Webview 与 Extension Host 的隔离边界，HostBridge 在 Extension Host 内直接分发。
...
这就是“核心定义自己需要什么，VS Code 适配器提供怎么实现”的端口—适配器结构。保留该边界仍有利于测试和限制 `vscode` API 向核心扩散。
...
| `DiffViewProvider` | `VscodeDiffViewProvider` / `FileEditProvider` |
| `CommentReviewController` | `VscodeCommentReviewController` |
| `ITerminalManager` | `VscodeTerminalManager` |
| Host Bridge clients | VS Code 内部方法适配器 |
| `ApiHandler` | 多个云端/本地模型 Handler |
| `StorageContext` | 共享文件存储 |
...
1. `proto/cline/*.proto`：Webview 调用 Extension Host 中的应用服务。
2. `proto/host/*.proto`：Agent Core 调用 VS Code 宿主能力。
...
```text
Webview    -- cline.* ProtoBus --> Extension Host
Agent Core -- host.* 本地调用 --> VS Code 适配器
```
...
5. 生成 VS Code Host Bridge 类型和本地服务表。
...
`App.tsx` 根据 Context 中的视图状态显示 Onboarding、Settings、History、MCP、Worktrees 或 Chat。Account 页面、PostHog Provider 和 Cline Auth Context 已删除。
...
- ViewModel：`ExtensionStateContext` 和各类 hooks；
...
- 全量 Extension State；
- 流式 partial message；
- MCP Server 连接状态、工具、资源和 Prompt 更新；
- 模型列表更新；
...
- 创建和销毁当前 `Task`；
- 管理 `McpHub` 和 OCA 等模型专属认证服务；
- 初始化工作区与任务锁；
- 切换 Plan/Act 模式和 API Handler；
- 组装前端需要的 Extension State；
- 管理任务历史、本地设置和后台命令状态；
- 响应模型专属 OAuth、模型目录和 MCP Server 生命周期流程。
...
支持流式半成品的 Handler 还实现 `IPartialBlockHandler`。文件读取、写入、Shell、浏览器、MCP、Plan/Act 响应、完成任务等都被封装为独立命令对象。
...
```text
Session Override > Task Settings > Global Settings
```

Remote Config 的联网拉取、Schema、状态字段、账号绑定、刷新入口和兼容空分支均已删除，不再参与设置优先级。设置只来自本地 Session、Task 和 Global 三层。
...
这种设计让高频读取很快，并让扩展后端和 Webview 共享稳定的数据契约。它不是完整 Repository Pattern，因为业务对象并没有通过聚合仓储加载；更准确地说是“Typed State Store + Write-behind Cache”。
...
`McpHub` 支持 stdio、SSE、Streamable HTTP、OAuth、自动重连、文件热加载和工具/资源/Prompt 发现。MCP 工具在运行时进入 Agent 工具空间，是最接近真正插件系统的部分。
...
Skills 从全局或项目目录延迟发现。`UseSkillToolHandler` 在需要时读取 `SKILL.md` 内容并注入当前上下文，不需要重新编译 Core。远程 Skill 拉取和 Remote Config 分支均已删除。
...
| Strategy | `ApiHandler`、Terminal Manager、Diff Provider | 可替换实现 |
...
- esbuild 只生成 VS Code Extension 的 Node/CJS bundle；
- Vite 单独构建 React Webview；
- Proto 生成器保证 Webview、VS Code 消息总线和 Host Bridge 使用同一契约；
- 平台原生二进制和 Webview 静态资源由构建流程复制；
- `package.json` 的 `contributes` 声明 VS Code View、Commands、Menus 和 Keybindings；代码结构提取改用 VS Code Document Symbol Provider，不再打包 Tree-sitter/WASM。
...
4. Storybook：UI 组件视觉与交互开发；
5. Proto lint、TypeScript、Biome 和构建检查：契约与静态边界。
...
- Webview ProtoBus 和 HostBridge 都是插件内部边界，不是内部业务微服务拆分。
...
1. **宿主边界清晰**：Controller、Task、Prompt 和 Tool 不需要在各处直接调用 VS Code API。
...
`Controller` 同时管理任务、模型专属认证、MCP、状态和模型；`Task` 同时管理流、Prompt、Context、工具、审批、Hooks、持久化和重试。它们是明显的 God Object 风险点。
...
4. `proto/cline/*.proto` 与 `proto/host/*.proto`：系统边界；
5. `src/core/controller/grpc-handler.ts`：协议分发；
6. `src/core/controller/index.ts`：应用门面；
7. `src/core/task/index.ts`、`TaskState.ts`：Agent 工作流；
8. `ToolExecutor.ts`、`ToolExecutorCoordinator.ts`：工具管线；
9. `src/core/api/index.ts` 与 `providers/*`：模型策略；
10. `src/core/prompts/system-prompt`：Prompt 引擎；
11. `src/core/storage/StateManager.ts`：状态真相与持久化；
12. `webview-ui/src/App.tsx`、`Providers.tsx`、`ExtensionStateContext.tsx`：前端投影；
13. `scripts/generate-protobus-setup.mjs`：协议如何连接各端。
...
> **外层**通过 VS Code Extension 组合根装配宿主适配器；**中间层**通过 ProtoBus 暴露应用用例并同步状态；**内核层**由 Controller、Task、Prompt、Context 和 Tool Pipeline 完成 Agent 编排；**外围层**通过模型 Provider、MCP、Hooks、Skills、Subagents、终端和编辑器适配器扩展能力。整个系统是模块化单体，但在宿主边界、消息边界和供应商边界上系统性使用了端口与适配器思想。
