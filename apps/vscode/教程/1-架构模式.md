...
原 Remote Config 联网拉取、账号绑定和刷新入口已删除。`getRemoteConfigSettings()` 暂时返回空对象，仅用于兼容少数尚未拆除的类型和永远不会命中的旧分支，不再参与设置优先级。
...
Skills 从全局或项目目录延迟发现。`UseSkillToolHandler` 在需要时读取 `SKILL.md` 内容并注入当前上下文，不需要重新编译 Core。远程 Skill 拉取入口已经移除；源码中保留的 Remote Config 类型分支固定为空。
