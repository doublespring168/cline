...
退出码 `0` 表示命令通过。当前代码的部分集成测试可能打印 `HostProvider not setup` 等未处理异步警告；这是测试隔离和遥测初始化问题，不等同于测试断言失败，但正式维护时仍应修复这些警告。
...
当前 `.vscodeignore` 已排除 `.env`、大部分源码、测试、开发配置、Standalone 产物和 Webview 源码，并保留运行所需的 Bundle、Webview Build、图标和 Codicon 资源。
...
### 7.1 当前“教程”目录会被打入 VSIX

当前 `.vscodeignore` 没有排除 `教程/`，因此本文和其他教程文档可能进入最终 VSIX。若这些文档只用于内部分析，建议在 `.vscodeignore` 中加入：

```text
教程/**
```

本文只记录步骤，不自动修改 `.vscodeignore`。是否保留教程文档由最终产品需求决定。
