...
| `src/config.ts` | production/staging/local/selfHosted App/API Endpoint | Shared Config、文件系统 | 模型专属认证、Providers |
...
Remote Config 的联网读取、Schema、持久化和兼容空读取均已删除。
...
| `services/symbols` | 代码定义提取 | VS Code Document Symbol Provider | ListCodeDefinition Tool |
...
| `components/mcp` | MCP Server 配置、运行状态、Rich Response | Context、MCP RPC、Chat/Common |
| `components/history` | 历史列表和预览 | Task RPC、Context、Shared |
| `components/onboarding` | 首次启动与模型选择 | State/Model RPC、Provider 专属认证 |
| `components/welcome` | 本地模型配置完成后的欢迎入口 | Task/State RPC、Assets |
