...
1. `clearTask()`，确保旧 Task 不与新 Task 并存；
2. 读取 Auto Approve、终端模式、输出行数、默认 Profile、任务历史等本地设置；
3. 初始化 `WorkspaceRootManager`，确定单根或多根工作区；
4. 计算主工作目录 `cwd`；
5. 新任务使用当前时间戳作为 `taskId`，历史任务复用原 ID；
6. 加载该任务的 Task Settings，并合并请求携带的临时设置；
7. 构造 `Task`，注入 Controller、MCP、状态、工作区、回调和终端配置；
8. 把实例赋给 `controller.task`；
9. 新任务调用 `task.startTask()`，历史任务调用 `task.resumeTaskFromHistory()`。

此链路不再刷新远程配置，也不会启动聊天记录云同步。
