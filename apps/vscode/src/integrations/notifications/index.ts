...
export function setNotificationPlatformForTesting(
	mock: typeof platform | null,
): void {
...
function abbreviateNotificationMessage(
	message: string,
	maxLength: number,
): string {
...
export function createApprovalNotificationMessage(
	options: ApprovalNotificationOptions,
): string {
	const suffix = options.requiresExplicitApproval
		? EXPLICIT_APPROVAL_NOTIFICATION_SUFFIX
		: "";
	const availableMessageLength = Math.max(
		0,
		MAX_APPROVAL_NOTIFICATION_LENGTH - suffix.length,
	);
	const abbreviatedMessage = abbreviateNotificationMessage(
		options.message,
 ...
 */
export function buildWindowsToastNotificationScript(
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('coderX').Show($toast)
    `;
}
...
export async function showSystemNotification(
	...
	try {
		const { title = "coderX", message } = options;
