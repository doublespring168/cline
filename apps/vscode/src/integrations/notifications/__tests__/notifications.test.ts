...
describe("notifications", () => {
	...
	it("escapes single quotes for PowerShell single-quoted strings", () => {
		expect(
			notificationsModule.escapePowerShellSingleQuotedString("don't"),
		).to.equal("don''t");
	});
	...
	it("escapes quotes and backslashes for macOS notifications", async () => {
		...
		notificationsModule.setNotificationExecaForTesting(
			execaStub as unknown as Parameters<
				typeof notificationsModule.setNotificationExecaForTesting
			>[0],
		);
		const platformStub = sinon.stub().returns("darwin");
		notificationsModule.setNotificationPlatformForTesting(
			platformStub as unknown as Parameters<
				typeof notificationsModule.setNotificationPlatformForTesting
			>[0],
		);
		...
		expect(script).to.contain('with title "coderX \\"Agent\\""');
	...
	it("routes approval notifications through platform dispatch only when enabled", async () => {
		...
		await notificationsModule.showApprovalNotification(
			{ message: "npm install", requiresExplicitApproval: true },
			false,
		);
	...
	it("abbreviates long approval notification messages while preserving the explicit approval suffix", () => {
		...
		expect(
			notificationMessage.endsWith(" (explicit approval required)"),
		).to.equal(true);
	...
	it("encodes Windows PowerShell notifications before dispatch", async () => {
		...
		expect(decodedScript).to.contain(
			"$message = 'npm`install $(Start-Process calc) \"quoted\"'",
		);
	...
	it("encodes PowerShell commands as UTF-16LE base64", () => {
		const encoded =
			notificationsModule.encodePowerShellCommand("Write-Host 'hello'");
		expect(decodePowerShellEncodedCommand(encoded)).to.equal(
			"Write-Host 'hello'",
		);
	});
});
