...
type UpdateTaskHistoryFunction = (
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(
		...
		try {
			...
			if (
				...
				this.state.checkpointManagerErrorMessage?.includes(
			...
			const clineMessages =
				this.services.messageStateHandler.getClineMessages();
			...
			if (!isAttemptCompletionMessage) {
				...
				const lastFiveclineMessages = this.services.messageStateHandler
					.getClineMessages()
					.slice(-3);
				const lastCompletionResultMessage = findLast(
					lastFiveclineMessages,
	...
	async restoreCheckpoint(
		...
		try {
			const clineMessages =
				this.services.messageStateHandler.getClineMessages();
			const messageIndex =
				clineMessages.findIndex((m) => m.ts === messageTs) - (offset || 0);
			...
			switch (restoreType) {
				...
				case "workspace":
					if (!this.config.enableCheckpoints) {
						...
						Logger.error(
							`[TaskCheckpointManager] ${errorMessage} for task ${this.task.taskId}`,
						);
					...
					if (
						...
						try {
							...
							this.services.messageStateHandler.setCheckpointTracker(
								this.state.checkpointTracker,
							);
						} catch (error) {
							const errorMessage =
					...
					if (message.lastCheckpointHash && this.state.checkpointTracker) {
						try {
							await this.state.checkpointTracker.resetHead(
			...
			if (!didWorkspaceRestoreFail) {
				await this.handleSuccessfulRestore(
					restoreType,
					message,
					messageIndex,
					messageTs,
				...
				if (this.state.checkpointManagerErrorMessage !== undefined) {
					checkpointManagerStateUpdate.checkpointManagerErrorMessage =
						this.state.checkpointManagerErrorMessage;
				}
			}
	...
	async presentMultifileDiff(
		...
		try {
			...
			if (!this.state.checkpointTracker) {
				Logger.error(
					`[TaskCheckpointManager] Checkpoint tracker not available for task ${this.task.taskId}`,
				);
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler
						.getClineMessages()
				...
				const firstCheckpointMessageCheckpointHash =
					this.services.messageStateHandler
						.getClineMessages()
				...
				changedFiles = await this.state.checkpointTracker.getDiffSet(
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			if (!message) {
				Logger.error(
					`[TaskCheckpointManager] Completion message not found for task ${this.task.taskId}`,
				);
				return false;
			}
			const hash = message.lastCheckpointHash;
			...
			if (!previousCheckpointHash) {
				Logger.error(
					`[TaskCheckpointManager] No previous checkpoint hash found for task ${this.task.taskId}`,
				);
				return false;
			}
			...
			const changedFilesCount =
				(await this.state.checkpointTracker.getDiffCount(
					previousCheckpointHash,
					hash,
				)) || 0;
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const apiConversationHistory =
					this.services.messageStateHandler.getApiConversationHistory();
				...
				await this.services.messageStateHandler.overwriteApiConversationHistory(
				...
				if (restoreType === "task") {
					const filesEditedAfterMessage =
						await this.services.fileContextTracker.detectFilesEditedAfterMessage(
							messageTs,
							deletedMessages,
						);
					if (filesEditedAfterMessage.length > 0) {
						await this.services.fileContextTracker.storePendingFileContextWarning(
							filesEditedAfterMessage,
						);
					}
				}

				const newClineMessages = clineMessages.slice(0, messageIndex + 1);
				await this.services.messageStateHandler.overwriteClineMessages(
					newClineMessages,
				); // calls saveClineMessages which saves historyItem
	 ...
	 */
	async checkpointTrackerCheckAndInit(): Promise<
	...
	> {
		...
		this.state.checkpointTrackerInitPromise =
			this.initializeCheckpointTracker();
	 ...
	 */
	private async initializeCheckpointTracker(): Promise<
		CheckpointTracker | undefined
	> {
		...
		try {
			checkpointsWarningTimer = setTimeout(async () => {
				if (!checkpointsWarningShown) {
					...
					await this.setcheckpointManagerErrorMessage(
						"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening coderX in a project that uses git, or disabling checkpoints.",
					);
				}
			}, 7_000);
			...
			const tracker = await pTimeout(
				...
				),
				{
					milliseconds: 15_000,
					message:
						"Checkpoints taking too long to initialize. Consider re-opening coderX in a project that uses git, or disabling checkpoints.",
				},
			);
			...
			if (errorMessage.includes("Checkpoints taking too long to initialize")) {
				await this.setcheckpointManagerErrorMessage(
					"Checkpoints initialization timed out. Consider re-opening coderX in a project that uses git, or disabling checkpoints.",
				);
			} else {
				await this.setcheckpointManagerErrorMessage(errorMessage);
			}
	 ...
	 */
	async setcheckpointManagerErrorMessage(
	 ...
	 */
	updateConversationHistoryDeletedRange(
	...
	private async getWorkspacePath(): Promise<string> {
		...
		const { getWorkingDirectory: getWorkingDirectoryImpl } = await import(
			"./CheckpointUtils"
		);
