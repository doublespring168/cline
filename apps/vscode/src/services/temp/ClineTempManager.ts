/**
 * ClineTempManager - Manages temporary files for coderX with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "coderx" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for coderX's temporary files.
 */
class ClineTempManagerImpl {
	...
	constructor() {
		// Uses system temp directory with a dedicated "coderx" subdirectory when possible.
		const baseTempDir = os.tmpdir();
		const coderxTempDir = path.join(baseTempDir, "coderx");

		try {
			fs.mkdirSync(coderxTempDir, { recursive: true });
			this.tempDir = coderxTempDir;
		} catch {
	...
	/**
	 * Clean up old coderX temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the coderX temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(
					`coderX temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`,
				);
			}
		} catch (error) {
			Logger.error("Error during coderX temp cleanup", error);
		}
