...
import "should";
import {
	type GetCallbackUrlFn,
	McpOAuthRedirectResolver,
} from "../McpOAuthRedirectResolver";

describe("McpOAuthRedirectResolver", () => {
	it("reuses a registration when the VS Code callback URL is unchanged", async () => {
		const savedUrl = "vscode://coderx.coderx/mcp-auth/callback/abc123";
		const getCallbackUrl: GetCallbackUrlFn = async (path) =>
			`vscode://coderx.coderx${path}`;
	...
	it("requires registration when no callback URL has been saved", async () => {
		const callbackUrl =
			"https://codespace.example.dev/mcp-auth/callback/abc123";
	...
	it("requires registration when a VS Code web callback URL changes", async () => {
		const savedUrl =
			"https://old-codespace.example.dev/mcp-auth/callback/abc123";
		const currentUrl =
			"https://new-codespace.example.dev/mcp-auth/callback/abc123";
