...
describe("McpHub.deleteServerRPC", () => {
	...
	beforeEach(async () => {
		...
		tempDir = path.join(
			os.tmpdir(),
			`mcp-delete-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		await fs.mkdir(tempDir, { recursive: true });
		settingsPath = path.join(tempDir, "coderx_mcp_settings.json");
