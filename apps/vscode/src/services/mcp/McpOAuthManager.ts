...
/**
 * Implementation of OAuthClientProvider for coderX
...
class ClineOAuthClientProvider implements OAuthClientProvider {
	...
	async initialize(): Promise<void> {
		...
		const callbackPath = getMcpServerCallbackPath(
			this.serverName,
			this.serverUrl,
		);
		const secrets = getMcpOAuthSecrets();
		const savedRedirectUrl =
			secrets[this.serverHash]?.redirect_url_at_registration;
		...
		if (serverData.tokens_saved_at && serverData.tokens.expires_in) {
			...
			if (serverData.tokens_saved_at + expiresInMs < Date.now()) {
				...
				if (serverData.tokens.refresh_token) {
					Logger.log(
						`[McpOAuth] Token expired for ${this.serverName}, will attempt refresh`,
					);
	...
	async redirectToAuthorization(authorizationUrl: URL): Promise<void> {
		...
		if (existingTokens && existingTokens.access_token) {
			Logger.warn(
				`[McpOAuth] Preserving existing tokens for ${this.serverName}`,
			);
			return;
		}
...
export class McpOAuthManager {
	...
	validateAndClearState(serverHash: string, state: string): boolean {
		...
		if (serverData.oauth_state_timestamp) {
			if (
				Date.now() - serverData.oauth_state_timestamp >
