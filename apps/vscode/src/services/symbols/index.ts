...
async function formatDocumentSymbols(filePath: string): Promise<string | null> {
	...
	const symbols = await vscode.commands.executeCommand<SupportedSymbol[]>(
 ...
 * Language extensions remain responsible for parsing, so the coderX extension does
...
export async function listCodeDefinitionsTopLevel(
	...
	const allowedFiles = clineIgnoreController
		? clineIgnoreController.filterPaths(allFiles)
		: allFiles;
	let result = "";
	...
	for (const filePath of allowedFiles.slice(0, 50)) {
		if (
			...
			!clineIgnoreController.validateAccess(filePath)
		) {
			continue;
		}
		...
		try {
			...
			if (definitions) {
				const relativePath = path
					.relative(resolvedPath, filePath)
					.split(path.sep)
					.join("/");
				result += `${relativePath}\n${definitions}\n`;
			}
			...
			Logger.debug(
				`Failed to get VS Code document symbols for ${filePath}: ${String(error)}`,
			);
		}
	}
