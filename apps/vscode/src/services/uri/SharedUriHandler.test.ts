...
describe("SharedUriHandler", () => {
	...
	beforeEach(() => {
		...
		sandbox
			.stub(WebviewProvider, "getVisibleInstance")
			.returns(mockWebviewProvider);
	});
	...
	describe("handleUri", () => {
		describe("OpenRouter callback handling", () => {
			it("should successfully handle OpenRouter callback with code", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://coderx.coderx/openrouter?code=test123",
				);

				expect(result).to.be.true;
				sinon.assert.calledOnceWithExactly(
					handleOpenRouterCallbackStub,
					"test123",
				);
			});
			...
			it("should return false when OpenRouter code is missing", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://coderx.coderx/openrouter",
				);
			...
			it("should handle URL with plus signs in code parameter", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://coderx.coderx/openrouter?code=test+123+abc",
				);
		...
		describe("Unknown path handling", () => {
			it("should return false for unknown paths", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://coderx.coderx/unknown?param=value",
				);
			...
			it("should reject the removed coderX account callback", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://coderx.coderx/auth?idToken=jwt123",
				);
		...
		describe("Query parameter parsing", () => {
			it("should ignore unrelated query parameters", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://coderx.coderx/openrouter?code=test123&extra=param",
				);
			...
			it("should handle URL-encoded parameters", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://coderx.coderx/openrouter?code=token%20with%20spaces",
				);
		...
		describe("Different URI schemes", () => {
			...
			it("should handle HTTPS scheme URIs", async () => {
				const result = await SharedUriHandler.handleUri(
					"https://example.com/openrouter?code=test456",
				);
