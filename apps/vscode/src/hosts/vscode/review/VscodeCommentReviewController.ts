	...
	implements vscode.Disposable
{
	...
	private readonly avatarUri = vscode.Uri.file(
		path.join(
	...
	constructor() {
		...
		this.commentController = vscode.comments.createCommentController(
			ExtensionRegistryInfo.commentControllerId,
			"coderX AI Review",
		);
	...
	/**
	 * Keep the host's global Comments preference untouched. Changing
	 * comments.openView would affect Cline and every other review extension.
	 */
	...
	/**
	 * Handle adding the thread conversation to coderX's main chat
	 */
...
Please continue helping the user with their question about this code.`;
	...
	/**
	 * Close all tabs that use the coderX diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with the coderX diff scheme (opened during comment reveal)
				if (
