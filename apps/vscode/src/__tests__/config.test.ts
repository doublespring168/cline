...
describe("ClineEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		// Create .coderx directory
		await fs.mkdir(path.join(tempDir, ".coderx"), { recursive: true });
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(
				path.join(tempDir, ".coderx", "endpoints.json"),
		...
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			try {
				...
				error.message.should.containEql(
					"Cannot change environment in on-premise mode",
				);
			}
		});
	...
	describe("bundled endpoints.json behavior", () => {
		...
		let setVscodeHostProviderMock: (mock: {
		...
		beforeEach(async () => {
			...
			const hostProviderModule = await import(
		...
		it("should use bundled endpoints.json when available", async () => {
			...
			await fs.writeFile(
				path.join(bundledDir, "endpoints.json"),
				JSON.stringify(bundledConfig),
				"utf8",
			);
