...
const CLINE_FREE_MODEL_EXCEPTIONS = [
	"minimax-m2",
	"devstral-2512",
...
export function isClineFreeModelException(modelId: string): boolean {
	...
	return CLINE_FREE_MODEL_EXCEPTIONS.some((token) =>
		normalizedModelId.includes(token),
	);
}

/**
 * Filters OpenRouter model IDs based on provider-specific rules.
 * For coderX provider: excludes :free models (except known exception models)
 ...
 * @param allowedFreeModelIds Optional list of coderX free model IDs to keep visible
...
export function filterOpenRouterModelIds(
	...
	if (provider === "cline") {
		const allowedFreeIdSet = new Set(
			allowedFreeModelIds.map((id) => normalizeModelId(id)),
		);
		// For coderX provider: exclude :free models, but keep known exception models
	...
	// For OpenRouter and Vercel AI Gateway providers: exclude coderX-specific models
