...
e2e.describe("Diff Editor", () => {
	E2E_WORKSPACE_TYPES.forEach(({ title, workspaceType }) => {
		e2e.extend({
			...
			await inputbox.fill("[diff.test.ts] Hello, coderX!");
			await expect(inputbox).toHaveValue("[diff.test.ts] Hello, coderX!");
			...
			await expect(sidebar.getByText("Hello, coderX!")).toBeVisible(); // History with the previous sent message
			...
			await expect(
				page.getByText("test.ts: Original ↔ coderX's"),
			).toBeVisible();
