...
export class E2ETestHelper {
	...
	private static async getWindowDiagnostics(
		...
		const titles = await Promise.all(
			windows.map(async (window, index) => {
				...
				const title = await E2ETestHelper.withTimeout(
					window.title(),
					E2ETestHelper.WINDOW_DIAGNOSTIC_TIMEOUT_MS,
				);
				return title.timedOut
					? `#${index}: title timed out`
					: `#${index}: ${title.value || "<untitled>"}`;
			}),
		);
	...
	public static async closePageForTeardown(page: Page): Promise<void> {
		...
		const result = await E2ETestHelper.withTimeout(
			page.close(),
			E2ETestHelper.TEARDOWN_TIMEOUT_MS,
		).catch((error) => {
		...
		if (result.timedOut) {
			console.warn(
				`[e2e teardown] page.close() exceeded ${E2ETestHelper.TEARDOWN_TIMEOUT_MS}ms; continuing to app cleanup`,
			);
		}
	}

	public static async closeAppForTeardown(
		app: ElectronApplication,
	): Promise<void> {
		...
		const closeResult = await E2ETestHelper.withTimeout(
			app.close(),
			E2ETestHelper.TEARDOWN_TIMEOUT_MS,
		).catch((error) => {
		...
		const windowDiagnostics = await E2ETestHelper.getWindowDiagnostics(
			app,
		).catch((error) => `window diagnostics failed: ${error}`);
		console.warn(
		...
		try {
			if (process.kill("SIGKILL")) {
				const exited = await E2ETestHelper.waitForProcessExit(
					process,
					E2ETestHelper.PROCESS_EXIT_TIMEOUT_MS,
				);
				if (!exited) {
					console.warn(
			...
			console.warn(
				`[e2e teardown] SIGKILL failed for VS Code/Electron process: ${error}`,
			);
		}
		...
		try {
			if (
				...
				await E2ETestHelper.waitForProcessExit(
					process,
					E2ETestHelper.PROCESS_EXIT_TIMEOUT_MS,
				);
			}
			...
			console.warn(
				`[e2e teardown] SIGTERM failed for VS Code/Electron process: ${error}`,
			);
		}
	}
	...
	public async completeOnboarding(webview: Frame): Promise<void> {
		...
		const providerSelectorInput = webview.getByTestId(
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the coderX extension.
 *
 * Extended Playwright test configuration for coderX E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the coderX VS Code extension,
 ...
 * - `page`: Playwright Page object representing the main VS Code window with coderX sidebar opened
 * - `sidebar`: Playwright Frame object representing the coderX extension's sidebar iframe
 *
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and coderX sidebar open
 * - **sidebar**: Provides access to the coderX extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the coderX extension in development mode
	...
	.extend<{
		...
		openVSCode: async ({ userDataDir, channel }, use, testInfo) => {
			...
			await use(async (workspacePath: string) => {
				// Create isolated coderX data directory for this test
				const clineTestDir = mkdtempSync(path.join(os.tmpdir(), "coderx-e2e-"));
				...
				const app = await _electron.launch({
					...
					env: {
						...
						TEMP_PROFILE: "true",
						E2E_TEST: "true",
						CODERX_ENVIRONMENT: "local",
						CODERX_DIR: clineTestDir, // Isolate test data from user's ~/.coderx
	...
	.extend<{ app: ElectronApplication; clineTestDir: string }>({
		app: async (
			...
			try {
				...
				// Clean up the isolated coderX data directory
				// Find all temp directories matching our pattern
				const tmpDir = os.tmpdir();
				try {
					const entries = readdirSync(tmpDir);
					for (const entry of entries) {
						if (entry.startsWith("coderx-e2e-")) {
							cleanupTasks.push(
