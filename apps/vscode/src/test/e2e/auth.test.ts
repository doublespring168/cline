...
e2e("Onboarding - configures a local API key without a Cline account", async ({ sidebar }) => {
	await expect(sidebar.getByText("Configure a model provider")).toBeVisible()
	await expect(sidebar.getByText("No Cline account is required.")).toBeVisible()
	await expect(sidebar.getByText(/Login to Cline|Create my Account|Sign Up with Cline/)).toHaveCount(0)
	...
	await expect(sidebar.getByText("Configure a model provider")).not.toBeVisible()
