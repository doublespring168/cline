...
e2e(
	"Chat - can send messages and switch between modes",
	async ({ helper, sidebar }) => {
		...
		await expect(inputbox).toHaveValue("Hello, coderX!");
		...
		await expect(sidebar.getByText("Hello, coderX!")).toBeVisible();
		...
		await expect(inputbox).toHaveValue(
			"/newtask following text should be preserved",
		);
		...
		await inputbox.pressSequentially("following text should be preserved");
		await expect(inputbox).toHaveValue(
			"@problems following text should be preserved",
		);
	},
);
