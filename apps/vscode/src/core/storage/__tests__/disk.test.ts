...
describe("disk - hooks functionality", () => {
	...
	beforeEach(async () => {
		...
		tempDir = path.join(
			os.tmpdir(),
			`disk-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	describe("getWorkspaceHooksDirs", () => {
		...
		it("should return hooks directory when it exists", async () => {
			...
			const workspaceRoot = path.join(tempDir, "workspace1");
			const hooksDir = path.join(workspaceRoot, ".coderxrules", "hooks");
		...
		it("should not return hooks directory if it's a file instead of directory", async () => {
			...
			const workspaceRoot = path.join(tempDir, "workspace1");
			const hooksPath = path.join(workspaceRoot, ".coderxrules", "hooks");
		...
		it("should return multiple hooks directories for multi-root workspace", async () => {
			...
			const hooksDir1 = path.join(workspaceRoot1, ".coderxrules", "hooks");
			const hooksDir2 = path.join(workspaceRoot2, ".coderxrules", "hooks");
		...
		it("should return only existing hooks directories in multi-root workspace", async () => {
			...
			const hooksDir1 = path.join(workspaceRoot1, ".coderxrules", "hooks");
			const hooksDir3 = path.join(workspaceRoot3, ".coderxrules", "hooks");
			...
			result.should.not.containEql(
				path.join(workspaceRoot2, ".coderxrules", "hooks"),
			);
		});
		...
		it("should use correct path joining for hooks directory", async () => {
			const workspaceRoot = path.join(tempDir, "workspace1");
			const expectedHooksDir = path.join(
				workspaceRoot,
				".coderxrules",
				"hooks",
			);
			...
			result[0].should.match(/\.coderxrules[\\/]hooks$/);
		});
...
describe("disk - atomic writes", () => {
	...
	before(async () => {
		...
		testGlobalStorageDir = path.join(
			os.tmpdir(),
			`cline-test-storage-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	describe("writeTaskHistoryToState and readTaskHistoryFromState", () => {
		...
		it("should overwrite existing task history", async () => {
			...
			const newItems = [
				createTestHistoryItem("new-1", "New task 1"),
				createTestHistoryItem("new-2", "New task 2"),
			];
	...
	describe("atomic write failure scenarios", () => {
		it("should leave original file intact if temp file write fails", async () => {
			// Write initial data
			const initialItems = [
				createTestHistoryItem("original-1", "Original task"),
			];
			await writeTaskHistoryToState(initialItems);
		...
		it("should leave original file intact if rename fails", async () => {
			// Write initial data
			const initialItems = [
				createTestHistoryItem("original-2", "Original task 2"),
			];
			await writeTaskHistoryToState(initialItems);
		...
		it("should ignore temp files during read operations", async () => {
			...
			const stateDir = await ensureStateDirectoryExists();
			const corruptTempPath = path.join(
				stateDir,
				"taskHistory.json.tmp.12345.corrupt",
			);
		...
		it("should handle concurrent read during write without corruption", async () => {
			// Write initial data
			const initialItems = [
				createTestHistoryItem("concurrent-read-1", "Initial task"),
			];
			await writeTaskHistoryToState(initialItems);
		...
		it("should handle partial temp file from interrupted process", async () => {
			...
			const initialItems = [
				createTestHistoryItem("partial-test-1", "Initial task"),
			];
			await writeTaskHistoryToState(initialItems);

			// Simulate an interrupted write by creating a partial temp file
			const stateDir = await ensureStateDirectoryExists();
			const partialTempPath = path.join(
				stateDir,
				"taskHistory.json.tmp.99999.partial",
			);

			// Write only part of a valid JSON array
			await fs.writeFile(
				partialTempPath,
				'[{"id":"partial","ts":123456789',
				"utf8",
			);
