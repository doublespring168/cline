...
export async function migrateEnableCheckpointsSetting(
	...
	const config = vscode.workspace.getConfiguration("coderx");
...
export async function migrateCustomInstructionsToGlobalRules(
	...
	try {
		...
		if (customInstructions?.trim()) {
			Logger.log("Migrating custom instructions to global coderX rules...");

			// Create global .coderxrules directory if it doesn't exist
			...
			Logger.log(
				"Successfully migrated custom instructions to global coderX rules",
			);
		}
	} catch (error) {
		Logger.error(
...
export async function migrateWelcomeViewCompleted(
	...
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const openAiCodexCredentials = await context.secrets.get(
				"openai-codex-oauth-credentials",
			);
