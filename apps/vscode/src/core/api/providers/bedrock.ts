...
export class AwsBedrockHandler implements ApiHandler {
	...
	async *createMessage(
		...
		if (baseModelId.includes("openai")) {
			yield* this.createOpenAIMessage(
				systemPrompt,
				messages,
				modelId,
				model,
		...
		if (baseModelId.includes("deepseek")) {
			yield* this.createDeepseekMessage(
				systemPrompt,
				messages,
				modelId,
				model,
	...
	private async getAwsCredentials(): Promise<{
		...
		const useProfile =
			(this.options.awsAuthentication === undefined &&
	...
	/**
	 * Converts coderX's tool definitions (Anthropic format with `input_schema`) to the
	 ...
	 */
	private mapClineToolsToBedrockToolConfig(
		tools?: ClineTool[],
		...
		const isAnthropicTool = (tool: ClineTool): tool is AnthropicTool =>
					...
					if (chunk.contentBlockStart) {
						...
						if (
							...
							if (blockIndex !== undefined) {
								...
								const signature =
									blockStart.start?.signature ||
									blockStart.contentBlock?.signature ||
									undefined;
	...
	private *handleBedrockStreamError(
		...
		if (chunk.internalServerException) {
			...
			const isContextError =
				/input.*too long|context.*exceed|maximum.*token|input length.*max.*tokens/i.test(
					message,
				);
	...
	private async *createAnthropicMessage(
		...
		const messagesWithCache = this.options.awsBedrockUsePromptCache
			? this.applyCacheControlToMessages(
					formattedMessages,
					lastUserMsgIndex,
		...
		const systemMessages = this.prepareSystemMessages(
			systemPrompt,
			this.options.awsBedrockUsePromptCache || false,
		);
		...
		const command = new ConverseStreamCommand({
			...
			inferenceConfig: this.getInferenceConfig(
				model.info,
				"anthropic",
				modelId,
			),
		...
		try {
			if (typeof item.source.data === "string") {
				...
				imageData = new Uint8Array(
					Buffer.from(item.source.data as Buffer | Uint8Array),
				);
			} else {
	...
	private async *createOpenAIMessage(
		...
		try {
			// Track token usage
			const inputTokenEstimate = this.estimateInputTokens(
				systemPrompt,
				messages,
			);
			let outputTokens = 0;
			...
			if (response.output?.message?.content) {
				for (const contentBlock of response.output.message.content) {
					...
					if (
						"reasoningContent" in contentBlock &&
						...
						if (
							...
							reasoning.reasoningText &&
							"text" in reasoning.reasoningText
						) {
			...
			if (response.usage) {
				const actualInputTokens =
					response.usage.inputTokens || inputTokenEstimate;
				...
				const actualCost = calculateApiCostOpenAI(
					model.info,
					actualInputTokens,
					actualOutputTokens,
					0,
			...
			if (reasoningText) {
				...
				for (let i = 0; i < reasoningText.length; i += reasoningChunkSize) {
					const chunk = reasoningText.slice(
						i,
						Math.min(i + reasoningChunkSize, reasoningText.length),
					);
			...
			if (!response.usage) {
				const finalCost = calculateApiCostOpenAI(
					model.info,
					inputTokenEstimate,
					outputTokens,
					0,
	...
	private async *createQwenMessage(
		...
		try {
			...
			if (response.usage) {
				...
				// Report actual usage after processing content
				const actualCost = calculateApiCostQwen(
					model.info,
					actualInputTokens,
					actualOutputTokens,
					0,
			...
			// Report final usage if we didn't have actual usage data earlier
			if (!response.usage) {
				const finalCost = calculateApiCostQwen(
					model.info,
					inputTokenEstimate,
					outputTokens,
					0,
