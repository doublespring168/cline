	...
	async *createMessage(
		...
		try {
			const stream = await client.chat.completions.create({
				...
				max_completion_tokens: this.options.lmStudioMaxTokens
			...
			throw new Error(
				"Please check the LM Studio developer logs to debug what went wrong. You may need to load the model with a larger context length to work with coderX's prompts. Alternatively, try enabling Compact Prompt in your settings when working with a limited context window.",
			);
		}
	}
