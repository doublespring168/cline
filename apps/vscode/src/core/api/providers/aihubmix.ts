...
export class AIhubmixHandler implements ApiHandler {
	...
	private routeModel(
		modelName: string,
	): "anthropic" | "openai" | "gemini" | "openai-response" {
		const id = modelName || "";
	...
	private async *createAnthropicMessage(
		...
		// Sanitize messages to remove coderX-specific fields like call_id that are not allowed by Anthropic API
	...
	private async *createOpenaiResponseMessage(
		systemPrompt: string,
		messages: any[],
	): ApiStream {
