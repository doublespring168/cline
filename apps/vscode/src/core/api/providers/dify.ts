...
export class DifyHandler implements ApiHandler {
	...
	async *createMessage(
		...
		const requestBody = {
			...
			user: "coderx-user", // A unique user identifier
			files: [],
		};
		...
		try {
					...
					if (line.startsWith("data: ")) {
						...
						try {
							...
							if (parsed.conversation_id && !this.conversationId) {
								...
								Logger.log(
									"[DIFY DEBUG] Captured conversation_id:",
									this.conversationId,
								);
							}
							...
							if (parsed.event === "message") {
								Logger.log(
									"[DIFY DEBUG] Message event, answer:",
									parsed.answer,
								);
								...
								if (typeof parsed.answer === "string") {
									...
									Logger.log(
										"[DIFY DEBUG] Updated fullText length:",
										fullText.length,
									);
									yield {
										type: "text",
								...
								if (parsed.answer) {
									...
									Logger.log(
										"[DIFY DEBUG] Replaced fullText length:",
										fullText.length,
									);
									yield {
										type: "text",
								...
								throw new Error(
									`Dify API error: ${parsed.message || "Unknown error"}`,
								);
							} else if (
								parsed.event === "workflow_started" ||
								...
								parsed.event === "node_started" ||
								parsed.event === "node_finished"
							) {
								Logger.log(
									"[DIFY DEBUG] Node event:",
									parsed.event,
									parsed.data,
								);
								// These are informational events, continue processing
								...
								Logger.log(
									"[DIFY DEBUG] Ping event received, keeping connection alive.",
								);
								// Ping event, do nothing
							} else {
								Logger.log(
									"[DIFY DEBUG] Unknown event type:",
									parsed.event,
									"Full object:",
									parsed,
								);
							...
							Logger.warn(
								"[DIFY DEBUG] Failed to parse JSON:",
								data,
								"Error:",
								e,
							);
			...
			if (!hasYieldedContent) {
				...
				Logger.error(
					"[DIFY DEBUG] No content was yielded! Diagnostic info:",
					diagnosticInfo,
				);
				...
				if (fullText.trim()) {
					Logger.log(
						"[DIFY DEBUG] Yielding accumulated text as fallback:",
						fullText,
					);
					yield {
						type: "text",
	...
	private convertMessagesToQuery(
		systemPrompt: string,
		messages: ClineStorageMessage[],
	): string {
	 ...
	 * @param user User identifier (defaults to "coderx-user")
	 * @returns Promise with file upload response
	 */
	async uploadFile(
		file: Buffer,
		filename: string,
		user: string = "coderx-user",
	): Promise<DifyFileResponse> {
		...
		if (!response.ok) {
			const errorText = await response.text();
			throw new Error(
				`Dify file upload error: ${response.status} ${response.statusText} - ${errorText}`,
			);
		}
	...
	async stopGeneration(
		...
		const response = await fetch(
			`${this.baseUrl}/chat-messages/${taskId}/stop`,
			{
				method: "POST",
				headers: this.jsonHeaders(),
		...
		if (!response.ok) {
			const errorText = await response.text();
			throw new Error(
				`Dify stop generation error: ${response.status} ${response.statusText} - ${errorText}`,
			);
		}
	}
	...
	async getConversations(
		...
		if (!response.ok) {
			const errorText = await response.text();
			throw new Error(
				`Dify get conversations error: ${response.status} ${response.statusText} - ${errorText}`,
			);
		}
	...
	async deleteConversation(
		...
		if (!response.ok) {
			const errorText = await response.text();
			throw new Error(
				`Dify delete conversation error: ${response.status} ${response.statusText} - ${errorText}`,
			);
		}
	}
	...
	async renameConversation(
		...
		if (!response.ok) {
			const errorText = await response.text();
			throw new Error(
				`Dify rename conversation error: ${response.status} ${response.statusText} - ${errorText}`,
			);
		}
	...
	async submitMessageFeedback(
		...
		if (!response.ok) {
			const errorText = await response.text();
			throw new Error(
				`Dify submit feedback error: ${response.status} ${response.statusText} - ${errorText}`,
			);
		}
	}
