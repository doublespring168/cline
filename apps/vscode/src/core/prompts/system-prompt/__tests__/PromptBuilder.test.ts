...
describe("PromptBuilder", () => {
	...
	describe("build", () => {
		it("should build a complete prompt", async () => {
			...
			expect(result).to.include("You are coderX.");
		...
		it("should handle missing components gracefully", async () => {
			...
			try {
				...
				expect(warnSpy.calls[0][0]).to.include(
					"Warning: Component 'CAPABILITIES_SECTION' not found",
				);
				expect(warnSpy.calls[1][0]).to.include(
					"Warning: Component 'RULES_SECTION' not found",
				);
			} finally {
		...
		it("should apply component overrides", async () => {
			...
			const customComponents: ComponentRegistry = {
				...
				SYSTEM_INFO_SECTION: async (variant) => {
					let template =
						variant.componentOverrides?.SYSTEM_INFO_SECTION?.template ||
						"DEFAULT";
		...
		it("should resolve runtime placeholders", async () => {
			...
			const templateWithRuntime: PromptVariant = {
				...baseVariant,
				baseTemplate:
					"Hello {{USER_NAME}}! Working on {{PROJECT_TYPE}} project.\n\n{{TOOLS}}",
			};
		...
		it("should handle component errors gracefully", async () => {
			...
			try {
				...
				expect(warnSpy.calls[0][0]).to.include(
					"Failed to build component 'SYSTEM_INFO_SECTION'",
				);
			} finally {
				// Restore original Logger.warn
				Logger.warn = originalWarn;
