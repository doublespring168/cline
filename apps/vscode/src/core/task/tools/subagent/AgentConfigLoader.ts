...
/** Default Directory for agent configurations: ~/Documents/coderX/Agents */
...
export async function readAgentConfigsFromDisk(
	homeDir = os.homedir(),
): Promise<Map<string, AgentBaseConfig>> {
	...
	try {
		...
		await Promise.all(
			yamlFiles.map(async (fileName) => {
				...
				try {
					...
					Logger.debug(
						`[AgentConfigLoader] Loaded agent config '${fileName}'`,
						parsed,
					);
					configs.set(normalizeAgentName(parsed.name), parsed);
				} catch (error) {
					Logger.error(
						`[AgentConfigLoader] Failed to parse agent config '${fileName}'`,
						error,
					);
				}
			}),
		...
		Logger.error(
			"[AgentConfigLoader] Failed to read agent configs from disk",
			error,
		);
		throw error;
	}
...
export type AgentConfigChangeListener = (
	configs: ReadonlyMap<string, AgentBaseConfig>,
	error?: Error,
) => void;
...
export class AgentConfigLoader {
	...
	private constructor(homeDir = os.homedir()) {
		...
		this.initialLoadPromise = this.load()
			...
			.catch((error) => {
				Logger.error(
					"[AgentConfigLoader] Failed to load initial agent configs",
					error,
				);
			})
			.finally(() =>
				this.watch().catch((error) =>
					Logger.error(
						"[AgentConfigLoader] Failed to start watching agent configs",
						error,
					),
				),
			);
	...
	public async load(): Promise<ReadonlyMap<string, AgentBaseConfig>> {
		...
		Logger.debug(
			`[AgentConfigLoader] Loaded ${configs.size} agent config(s) from disk.`,
		);
	...
	public async watch(listener?: AgentConfigChangeListener): Promise<void> {
		...
		this.watcher
			...
			.on("error", (error) => {
				...
				Logger.error(
					"[AgentConfigLoader] Failed to watch agent configs directory",
					watcherError,
				);
	...
	private async reloadAndNotify(): Promise<void> {
		try {
			...
			const parseError =
				error instanceof Error ? error : new Error(String(error));
			Logger.error(
				"[AgentConfigLoader] Failed to reload agent configs",
				parseError,
			);
	...
	private rebuildDynamicToolMappings(): void {
		const sortedConfigs = Array.from(this.cachedConfigs.entries()).sort(
		...
		setDynamicToolUseNames(
			SUBAGENT_DYNAMIC_TOOL_NAMESPACE,
			Array.from(toolNameToAgentName.keys()),
		);
	}
}
