...
You are a code reviewer.`;
	...
	it("supports raw coderX tool ids in tools", () => {
		const content = `---
name: cli-agent
...
Prompt body`;
	...
	it("loads all yaml/yml files from homeDir/Documents/coderX/Agents", async () => {
		const tempHome = await createTempHomeDir();
		tempDirs.push(tempHome);
...
Reviewer prompt`,
		...
		assert.equal(
			loader.resolveSubagentNameForTool(withToolNames[0].toolName),
			"code reviewer",
		);
