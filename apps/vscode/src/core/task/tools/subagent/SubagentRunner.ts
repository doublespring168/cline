...
import type { ClineAssistantToolUseBlock, ClineStorageMessage, ClineTextContentBlock, ClineUserContent } from "@shared/messages"
		...
		return {
			...
			callbacks: {
				...
				executeCommandTool: async (command: string, timeoutSeconds: number | undefined) => {
					...
					try {
						return await baseCallbacks.executeCommandTool(command, timeoutSeconds, { suppressUserInteraction: true })
					} finally {
