		...
		try {
			...
			if (hooksEnabled) {
				try {
					...
					const useAutoCondense =
						StateManager.get().getGlobalSettingsKey("useAutoCondense");
					const strategy = useAutoCondense
						? "auto-condense"
						: "standard-truncation-firstpair";
					...
					const result = await executePreCompactHookWithCleanup({
						...
						ulid: config.ulid,
						modelContext: getHookModelContext(
							config.api,
					...
					if (result.contextModification) {
						...
						Logger.log(
							`[PreCompact] Hook provided context modification for task ${config.taskId}`,
						);
					}
				} catch (error) {
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					// Check .coderxignore first and skip ignored files
					...
					if (
						...
						try {
							...
							Logger.error(
								`Failed to read ${relPath} during summarization:`,
								error,
							);
						}
					}
			...
			if (fileContents) {
				const fileMentionString =
					loadedFilePaths.map((path) => `'${path}'`).join(", ") +
