...
export class WriteToFileToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (block.name === "replace_in_file" && !rawDiff) {
			...
			await config.callbacks.say(
				"error",
				`coderX tried to use replace_in_file for '${relPath}' without value for required parameter 'diff'. Retrying...`,
			);
		...
		if (block.name === "write_to_file" && rawContent == null) {
			...
			await config.callbacks.say(
				"error",
				`coderX tried to use write_to_file for '${relPath}' without value for required parameter 'content'. ${
					config.taskState.consecutiveMistakeCount >= 2
						? "This has happened multiple times — coderX will try a different approach."
						: "Retrying..."
				}`,
			);
		...
		try {
			...
			// Mark the file as edited by coderX
	...
	async validateAndPrepareFileOperation(
		...
		if (diff) {
			...
			// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but coderX throws error
