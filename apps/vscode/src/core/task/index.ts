...
export class Task {
	...
	private async handleHookCancellation(
		...
		// Log normalized provider errors for debugging.
		Logger.log(
	...
	public async startTask(
		...
		// if the extension process were killed, then on restart the clineMessages might not be empty, so we need to set it to [] when we create a new coderX client (otherwise webview would show stale messages from previous session)
	...
	private async initiateTaskLoop(userContent: ClineContent[]): Promise<void> {
		...
		while (!this.taskState.abort) {
			...
			// this.say(
			// 	"tool",
			// 	"coderX responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
			// )
			nextUserContent = [
				{
					type: "text",
	...
	private async writePromptMetadataArtifacts(params: {
		...
		const enabledFlag =
			process.env.CODERX_WRITE_PROMPT_ARTIFACTS?.toLowerCase();
		const enabled =
		...
		try {
			const configuredDir = process.env.CODERX_PROMPT_ARTIFACT_DIR?.trim();
			const artifactDir = configuredDir
				? path.isAbsolute(configuredDir)
					? configuredDir
					: path.resolve(this.cwd, configuredDir)
				: path.resolve(this.cwd, ".coderx-prompt-artifacts");
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		try {
			...
			const clineError = ClineError.transform(error, model.id, providerId);
			Logger.error(
				"Model request failed before the first response chunk",
				clineError,
			);
	...
	async recursivelyMakeClineRequests(
		...
		if (
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message:
						"coderX is having trouble. Would you like to continue the task?",
				});
			}
			const { response, text, images, files } = await this.ask(
				"mistake_limit_reached",
				this.api.getModel().id.includes("claude")
					? `This may indicate a failure in coderX's thought process or inability to use a tool properly, which can be mitigated with some user guidance (e.g. "Try breaking down the task into smaller steps").`
					: "coderX uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use a stronger model with better tool-calling and agentic coding capabilities.",
			);
		...
		// error handling if the user uses the /newrule command & their .coderxrules is a file, for file read operations didnt work properly
		if (clinerulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.coderxrules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			);
		}
		...
		try {
			...
			*/
			...
			// OpenRouter/coderX may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantHasContent) {
				...
				// Minimal diagnostics for empty provider responses.
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that coderX cannot process. Retrying the request may help resolve this issue.";
