...
export async function parseMentions(
	...
	for (const mention of uniqueMentions) {
		...
		// and enforce strict size/.coderxignore limits instead.
		if (mention === "/") {
			continue;
		}
		...
		if (mention.startsWith("http")) {
			...
			if (isMultiRoot && !workspaceHint) {
				...
				if (successfulResults.length === 0) {
					...
					const totalLength = successfulResults.reduce(
				...
				if (!targetRoot) {
					...
					try {
						...
						let errorType: "not_found" | "permission_denied" | "unknown" =
							"unknown";
						if (
							error.message.includes("ENOENT") ||
							error.message.includes("Failed to access")
						) {
							errorType = "not_found";
						} else if (
 ...
 */
function parseWorkspaceMention(
	mention: string,
): { workspaceHint: string; path: string } | null {
...
function getFilePathFromMention(mention: string): string {
	...
	if (workspaceMention) {
		...
		return workspaceMention.path.startsWith("/")
			? workspaceMention.path.slice(1)
			: workspaceMention.path;
	}
