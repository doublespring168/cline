...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - clineVersion: Current coderX extension version
	 ...
	 * - userId: coderX user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: coderX User ID, machine ID, or generated UUID
			...params,
			model,
		};
	}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/coderX/Hooks/)
 * and workspace-specific hooks (from each workspace's .coderxrules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .coderxrules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Global hooks are located in paths containing "coderX/Hooks".
	 */
	private static isGlobalHooksDir(dir: string): boolean {
		return /[/\\]coderX[/\\]hooks/i.test(dir);
	}
	 ...
	 *
	 * - Global hooks (from ~/Documents/coderX/Hooks/): run from the primary workspace root
	 * - Workspace hooks (from workspaceRoot/.coderxrules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.coderxrules/hooks/
	 ...
	 * Global hooks are located in ~/Documents/coderX/Hooks/
	 * Workspace hooks are located in workspace .coderxrules/hooks/ directories
	 *
	 ...
	 * Includes both global hooks (from ~/Documents/coderX/Hooks/) and workspace hooks
	 * (from .coderxrules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .coderxrules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .coderxrules directory path to search
