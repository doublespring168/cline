...
/**
 * Builds the evaluation context used for conditional coderX Rules (e.g. YAML frontmatter `paths:`).
 *
...
export class RuleContextBuilder {
	...
	static async buildEvaluationContext(
		deps: RuleContextBuilderDeps,
	...
	private static async getRulePathContext(
		deps: RuleContextBuilderDeps,
	): Promise<string[]> {
		...
		const lastUserMsg = [...clineMessages]
			...
			.find(
				(m) =>
					m.type === "say" &&
					(m.say === "user_feedback" || m.say === "task") &&
		...
		// (2) Visible + open tabs
		const roots = deps.workspaceManager?.getRoots().map((r) => r.path) ?? [
			deps.cwd,
		];
		const rawVisiblePaths =
			(await HostProvider.window.getVisibleTabs({}))?.paths ?? [];
		const rawOpenTabPaths =
		...
		// (3) Files edited by coderX during this task (completed operations):
		...
		for (const msg of clineMessages) {
			...
			try {
				...
				if (tool.tool === "applyPatch" && tool.content) {
					candidates.push(
						...RuleContextBuilder.extractPathsFromApplyPatch(tool.content),
					);
				}
			} catch {
				// ignore parse errors
		...
		for (const c of candidates) {
			...
			if (normalized.length >= RuleContextBuilder.MAX_RULE_PATH_CANDIDATES)
				break;
		}
