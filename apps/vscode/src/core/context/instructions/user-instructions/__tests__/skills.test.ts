...
import { discoverSkills, filterEnabledSkills, getAvailableSkills, getSkillContent } from "../skills"
...
describe("Skills Utility Functions", () => {
	...
	async function createProjectSkill(name: string, description?: string, body = "Instructions") {
		const skillDir = path.join(cwd, ".cline", "skills", name)
	...
	it("filters disabled local and global skills independently", () => {
		const skills: SkillMetadata[] = [
			{ name: "local", description: "Local", path: "/local/SKILL.md", source: "project" },
			{ name: "global", description: "Global", path: "/global/SKILL.md", source: "global" },
		]
	...
	it("loads and trims skill instructions from disk", async () => {
		await createProjectSkill("format", "Formats code", "  Use the formatter.  \n")
