...
describe("rule loading with paths frontmatter", () => {
	it("filters rules by evaluationContext.paths", async () => {
		...
		try {
			const rulesDir = path.join(tmp, ".coderxrules");
			...
			const res1 = await getRuleFilesTotalContentWithMetadata(
				files,
				rulesDir,
				toggles,
				{
			...
			expect(res1.activatedConditionalRules.map((r) => r.name)).to.include(
				"global:scoped.md",
			);

			const res2 = await getRuleFilesTotalContentWithMetadata(
				files,
				rulesDir,
				toggles,
				{
	...
	it("treats invalid YAML frontmatter as fail-open and preserves the raw frontmatter for the LLM", async () => {
		...
		try {
			...
			const res = await getRuleFilesTotalContentWithMetadata(
				files,
				rulesDir,
				toggles,
				{
	...
	it("treats paths: [] as match-nothing (fail-closed)", async () => {
		...
		try {
			...
			await fs.mkdir(rulesDir, { recursive: true });
			await fs.writeFile(
				path.join(rulesDir, "scoped-empty.md"),
	...
	it("keeps activatedConditionalRules order stable (matches input file order)", async () => {
		...
		try {
			...
			expect(res.activatedConditionalRules.map((r) => r.name)).to.deep.equal(
				files.map((f) => `global:${f}`),
			);
		} finally {
			await fs.rm(tmp, { recursive: true, force: true });
		}
