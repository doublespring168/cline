...
async function loadSkillMetadata(
	...
	try {
		...
		if (frontmatter.name !== skillName) {
			Logger.warn(
				`Skill name "${frontmatter.name}" doesn't match directory "${skillName}"`,
			);
			return null;
		}
...
/**
 * Discover all skills from global (~/.coderx/skills) and project directories.
 *
...
export async function discoverSkills(cwd: string): Promise<SkillMetadata[]> {
	...
	// Insert in order: project → disk-global. getAvailableSkills iterates backwards,
	// so global skills take precedence over project skills with the same name.
