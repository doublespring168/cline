...
// If a user modifies a file outside of coderX, the context may become stale and need to be updated.
// We do not want coderX to reload the context every time a file is modified, so we use this class merely
// to inform coderX that the change has occurred, and tell coderX to reload the file before making
// any changes to it. This fixes an issue with diff editing, where coderX was unable to complete a diff edit.
// a diff edit because the file was modified since coderX last read it.

// FileContextTracker
/**
This class is responsible for tracking file operations.
If the full contents of a file are passed to coderX via a tool, mention, or edit, the file is marked as active.
If a file is modified outside of coderX, we detect and track this change to prevent stale context.
...
export class FileContextTracker {
	...
	async setupFileWatcher(filePath: string) {
		...
		watcher.on("change", () => {
			if (this.recentlyEditedByCline.has(filePath)) {
				this.recentlyEditedByCline.delete(filePath); // This was an edit by coderX, no need to inform coderX
			} else {
				this.recentlyModifiedFiles.add(filePath); // This was a user edit, we will inform coderX
	 ...
	 * This is the main entry point for FileContextTracker and is called when a file is passed to coderX via a tool, mention, or edit.
	 */
	...
	async addFileToFileContextTracker(
		...
		try {
			...
			switch (source) {
				...
				// cline_edited: coderX has edited the file
				case "cline_edited":
					newEntry.cline_read_date = now;
					newEntry.cline_edit_date = now;
					break;

				// read_tool/file_mentioned: coderX has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.cline_read_date = now;
					break;
	...
	/**
	 * Marks a file as edited by coderX to prevent false positives in file watchers
	 */
	...
	async dispose(): Promise<void> {
		const closePromises = Array.from(this.fileWatchers.values()).map(
	...
	/**
	 * Detects files that were edited by coderX or users after a specific message timestamp
	...
	async detectFilesEditedAfterMessage(
		...
		try {
			// Check task metadata for files that were edited by coderX or users after the message timestamp
			...
			if (taskMetadata?.files_in_context) {
				for (const fileEntry of taskMetadata.files_in_context) {
					const clineEditedAfter =
						fileEntry.cline_edit_date && fileEntry.cline_edit_date > messageTs;
					const userEditedAfter =
						fileEntry.user_edit_date && fileEntry.user_edit_date > messageTs;
	...
	async retrievePendingFileContextWarning(): Promise<string[] | undefined> {
		try {
			...
			// NOTE: Using 'as any' because dynamic keys like pendingFileContextWarning_${taskId}
			// are legitimate workspace state keys but don't fit the strict LocalStateKey type system
			const files = this.controller.stateManager.getWorkspaceStateKey(
				key as any,
			) as string[];
			return files;
		} catch (error) {
	 ...
	 */
	async retrieveAndClearPendingFileContextWarning(): Promise<
		string[] | undefined
	> {
		try {
	 ...
	 */
	static async cleanupOrphanedWarnings(
		...
		try {
			...
			const allStateKeys = Object.keys(
			...
			Logger.error(
				"[FileContextTracker] Error cleaning up orphaned file context warnings:",
				error,
			);
		}
	}
