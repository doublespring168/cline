import { ensureCoderXSkillsDirectoryExists } from "@core/storage/disk";
...
export async function createSkillFile(
	...
	if (isGlobal) {
		// Create in ~/.coderx/skills using the isolated coderX helper
		const globalSkillsDir = await ensureCoderXSkillsDirectoryExists({
			isGlobal: true,
		});
		...
		// Create in .coderx/skills using the isolated coderX helper
		const localSkillsDir = await ensureCoderXSkillsDirectoryExists({
			isGlobal: false,
