...
describe("supportsReasoningEffortForModel", () => {
	it("should return true for OpenRouter/coderX model families that use reasoning effort", () => {
	...
	it("should return false for undefined and unrelated model IDs", () => {
		...
		supportsReasoningEffortForModel("anthropic/claude-sonnet-4.5").should.equal(
			false,
		);
	});
});
...
describe("isPoolsideModelFamily", () => {
	...
	it("should qualify Laguna models for next-gen and native tool calling paths", () => {
		...
		isNativeToolCallingConfig(
			providerInfo("openai-compatible", "poolside/laguna-m.1"),
			true,
		).should.equal(true);
		isNativeToolCallingConfig(
			providerInfo("openai-compatible", "poolside/laguna-m.1"),
...
describe("isGeminiFlashModel", () => {
	...
	it("should return false for non-matching IDs", () => {
		isGeminiFlashModel("openrouter/google/gemini-2.5-flash").should.equal(
			false,
		);
		isGeminiFlashModel("google/gemini-2.5-pro").should.equal(false);
	});
...
describe("modelDoesntSupportWebp", () => {
	...
	it("should return true for Devstral models running through llama.cpp", () => {
		...
		modelDoesntSupportWebp(m("Devstral-Small-2-24B-Instruct")).should.equal(
			true,
		);
	});
