...
describe("Filesystem Utilities", () => {
	...
	it("should correctly handle complex nested directory structures", async () => {
		...
		await fs.writeFile(
			path.join(complexDir, "dir2", "subdir1", "file3.txt"),
			"content",
		);
		...
		await fs.writeFile(
			path.join(complexDir, "dir3", "subdir2", "file5.txt"),
			"content",
		);
		await fs.writeFile(
			path.join(complexDir, "dir3", "subdir2", "deepdir", "file6.txt"),
			"content",
		);
	...
	it("should exclude .coderxrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const clinerulesDirTest = path.join(tmpDir, "clinerules-test");
		const clinerulesDirPath = path.join(clinerulesDirTest, ".coderxrules");

		// Create .coderxrules directory and root files
		await fs.mkdir(clinerulesDirPath, { recursive: true });
		await fs.writeFile(path.join(clinerulesDirPath, "config.json"), "{}");
		await fs.writeFile(
			path.join(clinerulesDirPath, "settings.js"),
			"// settings",
		);

		// Create .coderxrules/other directory and files
		...
		// Create .coderxrules/workflows directory and files
		const workflowsDirPath = path.join(clinerulesDirPath, "workflows");
		await fs.mkdir(workflowsDirPath, { recursive: true });
		await fs.writeFile(
			path.join(workflowsDirPath, "workflow1.js"),
			"// workflow1",
		);
		await fs.writeFile(
			path.join(workflowsDirPath, "workflow2.js"),
			"// workflow2",
		);
		...
		const filteredFiles = await readDirectory(clinerulesDirPath, [
	...
	it("should exclude .coderxrules/hooks directory specifically", async () => {
		// Create a test directory structure
		...
		// Create .coderxrules/hooks directory and files
		const hooksDirPath = path.join(clinerulesDirPath, "hooks");
		await fs.mkdir(hooksDirPath, { recursive: true });
		await fs.writeFile(
			path.join(hooksDirPath, "PreToolUse"),
			"#!/usr/bin/env bash",
		);
		await fs.writeFile(
			path.join(hooksDirPath, "PostToolUse"),
			"#!/usr/bin/env bash",
		);
