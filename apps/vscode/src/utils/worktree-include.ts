 ...
 */
async function isDirectoryPattern(
	sourceDir: string,
	pattern: string,
): Promise<string | null> {
 ...
 */
async function copyDirectoryNative(
	source: string,
	target: string,
): Promise<void> {
...
/**
 * Copies files matched by .coderxworktreeinclude patterns that are also in .gitignore.
...
export async function copyWorktreeIncludeFiles(
	...
	// Read .coderxworktreeinclude file
	const worktreeIncludePath = path.join(sourceDir, ".coderxworktreeinclude");
	...
	for (const pattern of includePatterns) {
		...
		if (dirName) {
			...
			if (
				gitignoreMatcher.ignores(dirName) ||
				gitignoreMatcher.ignores(dirName + "/")
			) {
	...
	if (filePatterns.length > 0) {
		...
		const filesToCopy = allFiles.filter((file) => {
			...
			const isIncluded =
				fileMatcher.ignores(file) || includeMatcher.ignores(file);
			const isGitignored = gitignoreMatcher.ignores(file);
			return isIncluded && isGitignored;
		});

		if (filesToCopy.length > 0) {
			const result = await copyFilesInBatches(
				filesToCopy,
				sourceDir,
				targetDir,
			);
...
/**
 * Checks if a .coderxworktreeinclude file exists in the given directory
 */
export async function hasWorktreeInclude(dir: string): Promise<boolean> {
	try {
		await fs.access(path.join(dir, ".coderxworktreeinclude"));
		return true;
	} catch {
		return false;
	}
