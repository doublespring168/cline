...
export async function createWorktree(
	...
	try {
		...
		// Copy files matched by .coderxworktreeinclude (if it exists)
		...
		const createdWorktree = worktrees.find(
			(w) => w.path === absoluteWorktreePath,
		);

		let message = `Worktree created at ${worktreePath}`;
		if (copiedCount > 0) {
			message += ` (copied ${copiedCount} file${copiedCount === 1 ? "" : "s"} from .coderxworktreeinclude)`;
		}
...
export async function getAvailableBranches(cwd: string): Promise<BranchInfo> {
	...
	try {
		...
		const remoteBranches = remoteBranchSummary.all.filter(
			(b) => !b.includes("HEAD"),
		);
		...
		const availableLocalBranches = localBranches.filter(
			(b) => !usedBranches.has(b),
		);
