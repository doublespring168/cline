...
describe("Worktree Include Utilities", () => {
	...
	describe("hasWorktreeInclude", () => {
		it("should return true when .coderxworktreeinclude exists", async () => {
			const testDir = path.join(tmpDir, "has-include");
			await fs.mkdir(testDir, { recursive: true });
			await fs.writeFile(
				path.join(testDir, ".coderxworktreeinclude"),
				"node_modules/",
			);
		...
		it("should return false when .coderxworktreeinclude does not exist", async () => {
	...
	describe("copyWorktreeIncludeFiles", () => {
		it("should return empty result when no .coderxworktreeinclude file exists", async () => {
		...
		it("should return empty result when no .gitignore file exists", async () => {
			...
			await fs.mkdir(tgt, { recursive: true });
			await fs.writeFile(
				path.join(src, ".coderxworktreeinclude"),
				"node_modules/",
			);
		...
		it("should copy entire directories using native cp", async () => {
			...
			await fs.writeFile(
				path.join(src, "node_modules", "file.txt"),
			...
			const pkgExists = await fs
				.access(path.join(tgt, "node_modules", "pkg", "index.js"))
				.then(
					() => true,
					() => false,
				);
		...
		it("should only copy files that are in both .coderxworktreeinclude AND .gitignore", async () => {
			const src = path.join(tmpDir, "intersection-src");
			const tgt = path.join(tmpDir, "intersection-tgt");

			await fs.mkdir(src, { recursive: true });
			await fs.mkdir(tgt, { recursive: true });
			await fs.writeFile(path.join(src, ".coderxworktreeinclude"), "*.log");
